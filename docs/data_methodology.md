...
For the **backtest**, historical composition data from `data/compositions.parquet` is used to restrict the eligible universe on each rebalance date — preventing the backtest from ranking stocks that were not yet in the index at that time.
